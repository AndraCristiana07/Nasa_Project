...
import { Sun, FlareMarker } from "../src/components/Planets";
...
describe("Sun Component API", () => {
  ...
  it("fetches solar flares and renders the correct markers", async () => {
    ...
    axios.get.mockResolvedValue({ mockFlares });

    render(<Sun />);

    expect(axios.get).toHaveBeenCalledWith(
      expect.stringContaining("/api/solar-flares"),
    );
  ...
  it("pauses mission on flare hover and resumes on leave", () => {
    ...
    const reactPropsKey = Object.keys(haloMesh).find((key) =>
